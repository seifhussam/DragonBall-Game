...
public class BattleMeVsMe extends GameVeiw  implements Serializable  {
...
private int previousMeHealth;
private int previousFoeHealth;
...
private Thread tt1;
private ImageIcon GukooFront;

	public BattleMeVsMe(Game game, Battle battle,TournamentView parent ) { 
		super(game);
		this.parent = parent ; 
		/*
		((Fighter) battle.getMe()).getUltimateAttacks().add(new UltimateAttack("Seif " , 500)) ;
		((Fighter) battle.getFoe()).getUltimateAttacks().add(new UltimateAttack("Seif " , 500)) ; 
		((Fighter) battle.getMe()).getSuperAttacks().add(new SuperAttack ("Seif " , 500)) ;
		((Fighter) battle.getFoe()).getSuperAttacks().add(new SuperAttack("Seif " , 500)) ; 
		game.getPlayer().setSenzuBeans(2); 
		*/
		
		
		SaiyanFront =  new  ImageIcon(getClass().getResource("GokuSSFoe.png"));
		GukoFront =new JLabel(new ImageIcon(getClass().getResource("GokuFoe.png")));
		
		GukooFront =new ImageIcon(getClass().getResource("GokuFoe.png"));
		...
		if (((Fighter)battle.getFoe()) instanceof Earthling) { 
			FoeIcon = RoshyFront ; 
			
		}
		else if (((Fighter)battle.getFoe()) instanceof Saiyan) {
			FoeIcon = GukoFront ; 
		}
		else if (((Fighter)battle.getFoe()) instanceof Frieza) { 
			FoeIcon = freizaFront ; 
		}
		else if (((Fighter)battle.getFoe()) instanceof Majin) { 
			FoeIcon = BuuFront ; 
		}else if (((Fighter)battle.getFoe()) instanceof Namekian) { 
			FoeIcon = picoloFront ; 
		}
...
//					}
		...
		if (e.getType() == BattleEventType.NEW_TURN) {
			...
			if (((PlayableFighter)battle.getFoe())instanceof Saiyan) {
				if (!((Saiyan)battle.getFoe()).isTransformed() ) {
					FoeIcon.setIcon(GukooFront) ;
					validate() ;
				}
			}
			
			 MeProgressbar.setValue(((Fighter)battle.getMe()).getHealthPoints());
			...
			FoeProgressbar.setValue(((Fighter)battle.getFoe()).getHealthPoints());
			  FoeProgressbar.setString(""+((Fighter)battle.getFoe()).getHealthPoints());
			 if(FoeProgressbar.getValue()*100/((Fighter)battle.getFoe()).getMaxHealthPoints()  <= 75 && FoeProgressbar.getValue()*100/((Fighter)battle.getFoe()).getMaxHealthPoints() >25) {
				 FoeProgressbar.setForeground(Color.orange);
			 }
			...
			Runnable r = new Runnable() {
				...
				public void run() {
				 ...
				 validate();
					...
					UseP2.setEnabled(true); 
					AttackP2.setEnabled(true); 
					BlockP2.setEnabled(true) ; 
					AttacksList2.setEnabled(true);
